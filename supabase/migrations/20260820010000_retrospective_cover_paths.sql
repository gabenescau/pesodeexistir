-- Include the current cover URL/path in retrospective artwork data.
-- Catalog records may store a legacy URL in books.image or the Storage object
-- path in books.image_path.
...
begin
  foreach v_kind in array array['month', 'previousMonth', 'year']::text[] loop
    ...
    select nullif(b.image, ''), nullif(b.image_path, '')
      into v_image, v_image_path
      from public.books b
     where b.title = v_title
       and (nullif(b.image, '') is not null or nullif(b.image_path, '') is not null)
    ...
    if v_image_path is not null then
      v_payload := jsonb_set(v_payload, array[v_kind, 'topBook', 'image_path'], to_jsonb(v_image_path), true);
    end if;
  end loop;
  return v_payload;
end;
...
begin
  ...
  foreach v_kind in array array['month', 'previousMonth', 'year']::text[] loop
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      'title', ranked.title,
      'image', ranked.image,
      'image_path', ranked.image_path,
      'minutes', round(ranked.seconds::numeric / 60)::integer
    ) order by ranked.seconds desc, ranked.title asc), '[]'::jsonb)
      into v_books
      from (
        select b.title,
               nullif(b.image, '') as image,
               nullif(b.image_path, '') as image_path,
         ...
         group by b.id, b.title, b.image, b.image_path
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      'name', ranked.name,
      'minutes', round(ranked.seconds::numeric / 60)::integer
    ) order by ranked.seconds desc, ranked.name asc), '[]'::jsonb)
      into v_authors
      from (
