import { optimizeImageForUpload, shouldOptimizeImage } from "./image-optimize.js";
...
// Otimizacao pre-upload (somente arquivos NOVOS): redimensiona e converte
// para WebP antes de enviar ao Supabase, economizando Storage. Arquivos
// antigos ja armazenados nunca sao tocados. Qualquer falha cai para o
// arquivo original — o upload nunca quebra por causa da otimizacao.
async function maybeOptimizeImage(file, kind) {
  try {
    if (!shouldOptimizeImage({ type: file.type, size: file.size, kind })) return file;
    return await optimizeImageForUpload(file, kind);
  } catch {
    return file;
  }
}
...
export function secureUpload({ file, bucket, kind }) {
  ...
  const request = maybeOptimizeImage(file, kind)
    .then((optimizedFile) => uploadOnce({ file: optimizedFile, bucket, kind }))
