        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 p-0 sm:items-center sm:p-4">
          <div className="flex max-h-[92svh] w-full max-w-2xl flex-col overflow-hidden rounded-t-[14px] border border-[var(--border)] bg-[var(--bg-card)] shadow-[var(--shadow-sm)] sm:rounded-[14px]">
            <header className="flex items-center justify-between gap-3 border-b border-[var(--border)] px-4 py-3">
              ...
              <button type="button" onClick={() => { resetComposer(); setOpen(false); }} className="flex size-10 items-center justify-center rounded-[8px] text-[var(--text-muted)] hover:bg-[var(--hover-overlay)]">
                <X className="size-5" />
              </button>
            </header>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-4">
              <div className="relative">
                <textarea
                  ...
                  rows={6}
                  className="min-h-36 w-full resize-none rounded-[10px] border border-[var(--border)] bg-[var(--bg-canvas)] px-4 py-3 text-[15px] leading-relaxed text-[var(--text-primary)] outline-none placeholder:text-[var(--text-placeholder)] focus:border-[var(--border-strong)]"
                />
            ...
            <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-[var(--border)] px-4 py-3">
              ...
              <div className="flex items-center gap-1">
                ...
                <button type="button" onClick={() => { setText((current) => `${current}${current && !current.endsWith(" ") ? " " : ""}@`); setMention({ query: "", start: text.length, end: text.length + 1 }); textareaRef.current?.focus(); }} className="flex min-h-10 items-center gap-1.5 rounded-full px-3 text-xs text-[var(--text-muted)] hover:bg-[var(--hover-overlay)]">
                  <UserRound className="size-4" /> Marcar
                </button>
                <button type="button" onClick={() => setPollEnabled((value) => !value)} className="flex min-h-10 items-center gap-1.5 rounded-full px-3 text-xs text-[var(--text-muted)] hover:bg-[var(--hover-overlay)]">
                  <BarChart3 className="size-4" /> Enquete
                </button>
