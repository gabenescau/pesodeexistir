...
export function AuthorPage() {
  ...
  const isFav = isFavoriteAuthor(author.id);

  // Discussões = posts da comunidade que mencionam livros deste autor.
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Livros do autor (grid de capas) */}
      <div>
        <div className="mb-4 flex items-center gap-2">
          <BookOpen className="size-4 text-[var(--text-muted)]" />
          <h3 className="text-sm font-semibold text-[var(--text-primary)]">Obras disponíveis ({authorBooks.length})</h3>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {authorBooks.map((book) => (
              <button
                ...
                className="text-left"
              >
                <div className="aspect-[2/3] rounded-[12px] overflow-hidden border border-[var(--border)] bg-[var(--bg-card)] relative">
                  <img
                    src={book.image}
                    alt={book.title}
      ...
      {/* Discussões da comunidade sobre este autor */}
      <div>
        <div className="mb-4 flex items-center gap-2">
        ...
        {authorPosts.length > 0 ? (
          <div className="space-y-3">
            {authorPosts.slice(0, 20).map((post) => (
              ...
              >
                <p className="text-sm text-[var(--text-primary)] leading-relaxed">{post.text}</p>
                ...
                <p className="mt-2 text-[11px] text-[var(--text-muted)]">
                  {post.book?.title ? `Sobre: ${post.book.title}` : "Discussão geral"}
                </p>
              </div>
            ))}
          </div>
          ...
          <p className="text-sm text-[var(--text-muted)]">
            Ainda não há discussões sobre obras deste autor. Seja o primeiro a publicar na comunidade.
          </p>
        )}
      </div>
    </div>
