...
const ADMIN_OPERATIONS = new Set([
  "bootstrap",
  "author-create", "author-update", "author-delete", "author-restore",
...
// Trilha de auditoria para exclusoes irreversiveis (hard DELETE via
// service_role bypassa o RLS). Nao muda o comportamento: apenas registra
// quem excluiu o que, para investigar relatos de "livros que somem".
// O snapshot e lido ANTES do delete; falha na leitura nao bloqueia,
// falha no log nunca bloqueia o delete.
async function auditDelete({ table, id, actorId, snapshot }) {
  try {
    ...
    // Logging nunca pode quebrar a operacao administrativa.
  }
  return true;
}
...
export async function getAdminBootstrap(req, res) {
  ...
  const [books, adminBookAssets, authors, releases, categories, ratings] = await Promise.all([
    supabaseRequest("books?select=*&deleted_at=is.null&order=created_at.desc&limit=2000"),
    supabaseRequest("rpc/admin_book_pdf_assets", { method: "POST", body: "{}" }).catch(() => []),
    supabaseRequest("authors?select=*&deleted_at=is.null&order=name&limit=1000"),
...
export async function handleAdminAction(req, res) {
  ...
  switch (operation) {
    ...
    case "author-delete": {
      const id = uuid(payload.id, "id");
      const snapshot = await supabaseRequest(
        `authors?select=id,name&id=eq.${encodeURIComponent(id)}&limit=1`,
      ).catch(() => null);
      await auditDelete({ table: "authors", id, actorId: session.user.id, snapshot: snapshot?.[0] || null });
      // Soft-delete: nada e apagado de verdade (migration 20260821000000).
      // A leitura publica e o catalogo ignoram deleted_at; author-restore reverte.
      await supabaseRequest(`authors?id=eq.${encodeURIComponent(id)}`, {
        method: "PATCH",
        headers: { Prefer: "return=representation" },
        body: JSON.stringify({ deleted_at: new Date().toISOString() }),
      });
      return { deleted: true, softDeleted: true, audited: true };
    }
    case "author-restore": {
      const id = uuid(payload.id, "id");
      await auditDelete({ table: "authors", id, actorId: session.user.id, snapshot: { restored: true } });
      await supabaseRequest(`authors?id=eq.${encodeURIComponent(id)}`, {
        method: "PATCH",
        headers: { Prefer: "return=representation" },
        body: JSON.stringify({ deleted_at: null }),
      });
      return { restored: true, audited: true };
    }
    case "book-create":
    ...
    case "book-delete": {
      const id = uuid(payload.id, "id");
      const snapshot = await supabaseRequest(
        `books?select=id,title,author_id&id=eq.${encodeURIComponent(id)}&limit=1`,
      ).catch(() => null);
      await auditDelete({ table: "books", id, actorId: session.user.id, snapshot: snapshot?.[0] || null });
      // Soft-delete: nada e apagado de verdade (migration 20260821000000).
      // A leitura publica e o catalogo ignoram deleted_at; book-restore reverte.
      await supabaseRequest(`books?id=eq.${encodeURIComponent(id)}`, {
        method: "PATCH",
        headers: { Prefer: "return=representation" },
        body: JSON.stringify({ deleted_at: new Date().toISOString() }),
      });
    ...
    case "book-restore": {
      const id = uuid(payload.id, "id");
      await auditDelete({ table: "books", id, actorId: session.user.id, snapshot: { restored: true } });
      await supabaseRequest(`books?id=eq.${encodeURIComponent(id)}`, {
        method: "PATCH",
        headers: { Prefer: "return=representation" },
        body: JSON.stringify({ deleted_at: null }),
