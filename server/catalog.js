...
const RELEASE_SELECT = "id,book_id,release_date,visible,created_at,books(id,title,image,image_path,author_id,category,bio,deleted_at,authors(name))";
const CATEGORY_SELECT = "id,name,sort_order,created_at,updated_at";
const RATING_SELECT = "book_id,rating_sum,rating_count";
const PAGE_SIZE = 48;
const MAX_OFFSET = 5000;
// Tiebreakers por id: sem eles, linhas com o mesmo created_at/name voltam em
// ordem nao deterministica e a paginacao por offset "pula" itens quando novos
// registros sao inseridos (livros que "somem" entre paginas).
const BOOK_ORDER = "created_at.desc,id.desc";
const AUTHOR_ORDER = "name.asc,id.asc";
...
export async function getPublicCatalog(req) {
  ...
  const [books, authors, releases, categories, ratings] = await Promise.all([
    includeBooks
      ? supabaseRequest(`books?select=${BOOK_SELECT}&deleted_at=is.null&order=${BOOK_ORDER}&offset=${booksOffset}&limit=${PAGE_SIZE + 1}`)
      : Promise.resolve([]),
    includeAuthors
      ? supabaseRequest(`authors?select=${AUTHOR_SELECT}&deleted_at=is.null&order=${AUTHOR_ORDER}&offset=${authorsOffset}&limit=${PAGE_SIZE + 1}`)
      : Promise.resolve([]),
    includeShared
  ...
  return {
    books,
    authors,
    weeklyReleases: releases.filter((release) => !release.books?.deleted_at),
    categories,
    ratings,
    coverUrls,
    pageSize: PAGE_SIZE,
